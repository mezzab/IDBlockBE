...
app.listen(port, () => {
    ...
    app.post('/saveBlock', (req, res) => {
        ...
        return ipfs.files.add(testBuffer, function (err, file) {
            ...
            if (!dni || !hash) {
                const msg = 'Es necesario un dni y un hash para ejecutar la funcion del Smart Contract.'
                console.log('\n' + msg);
                return res.send(msg)
            }
            ...
            res.send({ code: 200, msg: 'Se guardo correctamente el hash en el smart contract' });
        });
    });
...
async function setHash(dni, hash) {
    ...
    try {
        const transaction = await contractInstance.methods.setHash(dni, hash).send({ from: accounts[0], gas: 150000 });
...
async function setRelation(dni, entityAddr) {
    ...
    try {
        const transaction = await contractInstance.methods.setRelation(dni, entityAddr).send({ from: accounts[0], gas: 150000 });
